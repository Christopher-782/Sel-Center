...
const viewMeta = {
  ...
  sales: [
    "Sales Management",
    "Create, review, correct or delete Kitchen and Ticket sales.",
  ],
  gate: [
    "Gate Entry Management",
    "Review, create, correct or delete gate-entry records.",
  ],
  tickets: [
    "Ticket Management",
    "Manage ticket types, selling prices and stock.",
  ],
  expenses: [
    "Expenses",
    "Record operating expenses used in net-profit calculation.",
  ],
  finance: [
    "Profit & Reports",
...
function notify(message, type = "success") {
  ...
  if (type !== "error")
    setTimeout(() => SELAccess.clearAlert(q("adminAlert")), 4200);
}

function showPanel(panel) {
  document
    .querySelectorAll(".side-link[data-panel]")
    .forEach((el) => el.classList.toggle("active", el.dataset.panel === panel));
  document
...
async function loadDashboard() {
  try {
    ...
    const [summary] = await Promise.all([
      ...
      inventory.length && tickets.length
        ? Promise.resolve()
        : Promise.all([loadInventory(false), loadTickets(false)]),
    ]);
...
function renderDashboardRecentSales() {
  const recent = [...sales]
    .sort(
      (a, b) =>
        new Date(b.sale_date || b.created_at) -
        new Date(a.sale_date || a.created_at),
    )
    .slice(0, 7);
  q("dashRecentSales").innerHTML = recent.length
    ? recent
        .map(
          (s) => `
    <tr>
      ...
      <td><strong>${money(s.total_amount)}</strong></td>
        ...
        .join("")
    : '<tr><td colspan="5" class="empty">No sales recorded yet.</td></tr>';
}
...
function renderLowStock() {
  const items = [
    ...inventory
      ...
      .map((x) => ({ name: x.name, qty: x.quantity, kind: "Kitchen" })),
    ...tickets
      .filter((x) => Number(x.quantity) <= Number(x.low_stock_threshold))
      .map((x) => ({ name: x.package_name, qty: x.quantity, kind: "Ticket" })),
  ]
    .sort((a, b) => Number(a.qty) - Number(b.qty))
    .slice(0, 8);
  q("lowStockList").innerHTML = items.length
    ? items
        .map(
          (x) => `
  ...
  `,
        ...
        .join("")
    : '<div class="empty-line">All tracked items are above their low-stock levels.</div>';
}

async function profileMap(ids = []) {
  const unique = [...new Set(ids.filter(Boolean))];
  if (!unique.length) return {};
  const { data } = await SELAccess.db()
    .from("profiles")
    .select("id,full_name,email")
    .in("id", unique);
  return Object.fromEntries(
    (data || []).map((p) => [p.id, p.full_name || p.email || "User"]),
  );
}
...
function gateAdminBounds() {
  ...
  if (!s || !e)
    throw new Error("Choose both start and end dates for the gate report.");
  const start = new Date(`${s}T00:00:00`);
  const end = new Date(`${e}T00:00:00`);
  end.setDate(end.getDate() + 1);
  if (end <= start)
    throw new Error("Gate report end date must be on or after the start date.");
  return { start, end };
}
...
async function loadGateAdmin(render = true) {
  try {
    ...
    if (render)
      notify(err.message || "Could not load gate-entry report.", "error");
  }
}
...
async function renderGateAdmin() {
  ...
  const rows = gateEntries.filter(
    (r) =>
      (payment === "all" || r.payment_mode === payment) &&
      (!search ||
        norm(`${r.receipt_no} ${r.payer_name || ""} ${r.notes || ""}`).includes(
          search,
        )),
  );
  ...
  const visitors = rows.reduce(
    (sum, r) => sum + Number(r.persons_count || 0),
    0,
  );
  const byPayment = (mode) =>
    rows
  ...
  q("gateAdminRows").innerHTML = rows.length
    ? rows
        .map(
          (r) => `<tr>
    ...
    <td><div class="row-actions"><button class="action-btn" onclick="openGateEditor(${r.id})" title="Edit gate entry"><i class="fa-solid fa-pen"></i></button><button class="action-btn danger" onclick="deleteGateEntry(${r.id})" title="Delete gate entry"><i class="fa-solid fa-trash"></i></button></div></td>
        ...
        .join("")
    : '<tr><td colspan="10" class="empty">No gate-entry records found for this filter.</td></tr>';
}
...
function gateCashEditorPreview() {
  ...
  if (
    pay === "cash" &&
    cashInput &&
    (!cashInput.value || Number(cashInput.value) < amount)
  )
    cashInput.value = amount ? amount.toFixed(2) : "";
  const change =
    pay === "cash" ? Math.max(0, Number(cashInput?.value || 0) - amount) : 0;
...
function openGateEditor(id = null) {
  ...
  const cashReceived =
    r?.payment_mode === "cash"
      ? Number(r.cash_received ?? r.amount ?? 0)
      : Number(r?.amount || 0);
  openDrawer(
    r ? "Edit Gate Entry" : "New Gate Entry",
    r
      ? `Correct ${r.receipt_no} while preserving its receipt reference.`
      : "Create a gate-entry record from the Admin Dashboard.",
    `
...
async function saveGateEntry(e) {
  ...
  try {
    ...
    const cashReceived =
      payment === "cash" ? Number(q("gateEditCashReceived").value || 0) : null;
    if (amount < 0) throw new Error("Gate amount cannot be negative.");
    if (payment === "cash" && cashReceived < amount)
      throw new Error(
        "Cash received cannot be less than the gate-entry amount.",
      );
    const payload = {
      amount,
    ...
    if (error) throw error;
    closeDrawer();
    notify(id ? "Gate entry updated." : "Gate entry created.");
    await loadGateAdmin();
    await loadDashboard();
  } catch (err) {
    notify(err.message || "Could not save gate entry.", "error");
  } finally {
    btn.disabled = false;
  }
}
...
async function deleteGateEntry(id) {
  ...
  if (
    !r ||
    !confirm(
      `Delete gate entry ${r.receipt_no} (${money(r.amount)})? This will remove it from revenue and reconciliation reports.`,
    )
  )
    return;
  const { error } = await SELAccess.db()
    .from("gate_fee_records")
    .delete()
    .eq("id", Number(id));
  if (error) return notify(error.message, "error");
  notify("Gate entry deleted.");
  await loadGateAdmin();
  await loadDashboard();
}
  ...
  return type === "kitchen"
    ? inventory
        .filter(
          (x) =>
            x.is_active ||
            saleDraftItems.some((i) => String(i.id) === String(x.id)),
        )
        .map((x) => ({
          id: x.id,
          name: x.name,
...
async function openSaleEditor(id = null) {
  ...
  saleDraftItems = sale
    ? (sale.sale_items || []).map((i) => ({
        ...
        unit_cost: sale.sale_type === "ticket" ? 0 : Number(i.unit_cost || 0),
      }))
    : [];
  const dateVal = localDateTimeValue(
    sale?.sale_date || sale?.created_at || new Date(),
  );
  openDrawer(
    sale ? "Edit Sale" : "New Sale",
    sale
      ? `Correct ${sale.sale_reference}. Stock will be reconciled automatically.`
      : "Record a manual Kitchen or Ticket sale.",
    `
  ...
  q("saleType").onchange = async (e) => {
    if (
      ...
      !confirm(
        "Changing the sale type will clear the selected items. Continue?",
      )
    ) {
      e.target.value = saleDraftType;
      return;
    }
...
function populateSaleProductSelect() {
  ...
  el.innerHTML = list.length
    ? list
        .map(
          (p) =>
            `<option value="${esc(p.id)}">${esc(p.name)} — ${money(p.price)} (${p.qty} available)</option>`,
        )
        .join("")
...
function addSaleDraftItem() {
  ...
  if (!id) return;
  const product = saleCatalog(saleDraftType).find(
...
function renderSaleDraft() {
  const list = q("saleDraftList");
  if (!list) return;
  const catalog = saleCatalog(saleDraftType);
  list.innerHTML = saleDraftItems.length
    ? saleDraftItems
        .map((i, idx) => {
        ...
        .join("")
    : '<div class="empty-line">Add at least one item to the sale.</div>';
  const total = saleDraftItems.reduce((sum, i) => {
    const p = catalog.find((x) => String(x.id) === String(i.id));
    return (
      sum + Number(i.unit_price ?? p?.price ?? 0) * Number(i.quantity || 0)
    );
  }, 0);
...
function updateAdminSaleCashPreview() {
  const fields = q("adminSaleCashFields"),
    payment = q("salePayment"),
    input = q("saleCashReceived"),
    preview = q("saleChangePreview");
  if (!fields || !payment || !input || !preview) return;
  const isCash = payment.value === "cash";
  fields.classList.toggle("hidden", !isCash);
  if (!isCash) {
    preview.textContent = money(0);
    return;
  }
  const total = saleDraftTotalAmount(),
...
function changeSaleDraftQty(index, delta) {
  ...
  saleDraftItems[index].quantity = Math.max(
    1,
    Number(saleDraftItems[index].quantity) + delta,
  );
  renderSaleDraft();
}

function removeSaleDraft(index) {
  saleDraftItems.splice(index, 1);
  renderSaleDraft();
}
...
async function saveSale(e) {
  ...
  if (!saleDraftItems.length)
    return notify("Add at least one item to the sale.", "error");
  const btn = e.submitter;
  btn.disabled = true;
  const id = q("saleId").value;
  const payload = saleDraftItems.map((i) => ({
    id: String(i.id),
    quantity: Number(i.quantity),
    unit_price: Number(i.unit_price || 0),
  ...
  const cashReceived =
    paymentMode === "cash" ? Number(q("saleCashReceived").value || 0) : null;
  if (paymentMode === "cash" && cashReceived < saleTotal) {
    btn.disabled = false;
    return notify("Cash received cannot be less than the sale total.", "error");
  }
  try {
    let result;
    if (id) {
    ...
    closeDrawer();
    notify(
      id ? "Sale updated and stock reconciled." : "Sale created successfully.",
    );
...
async function deleteSale(id) {
  ...
  if (
    !sale ||
    !confirm(
      `Delete sale ${sale.sale_reference}? Its item quantities will be returned to stock.`,
    )
  )
    return;
  const { error } = await SELAccess.db().rpc("admin_delete_sale", {
    p_sale_id: Number(id),
  });
  if (error) return notify(error.message, "error");
...
function renderTickets() {
  ...
  const rows = tickets.filter(
    (t) =>
      (audience === "all" || norm(t.audience) === audience) &&
      (!search ||
        norm(`${t.package_name} ${t.audience} ${t.ticket_type}`).includes(
          search,
        )),
  );
  q("ticketRows").innerHTML = rows.length
    ? rows
        .map(
          (t) => `
...
function openTicketEditor(id = null) {
  ...
  openDrawer(
    t ? "Edit Ticket" : "New Ticket",
    "Manage the ticket definition, pricing and stock.",
    `
...
async function saveTicket(e) {
  ...
  const payload = {
    package_name: q("ticketName").value.trim(),
    audience: q("ticketAudience").value,
    ticket_type: q("ticketType").value,
    price: Number(q("ticketPrice").value || 0),
    quantity: Number(q("ticketQty").value || 0),
    low_stock_threshold: Number(q("ticketLow").value || 0),
    description: q("ticketDescription").value.trim() || null,
    is_active: q("ticketActive").checked,
    updated_at: new Date().toISOString(),
  };
  const query = id
    ? SELAccess.db()
        ...
        .update(payload)
        .eq("id", Number(id))
    : SELAccess.db().from("ticket_packages").insert(payload);
  const { error } = await query;
  btn.disabled = false;
  if (error) return notify(error.message, "error");
  closeDrawer();
  notify(id ? "Ticket updated." : "Ticket created.");
  await loadTickets();
}
...
async function deleteTicket(id) {
  ...
  if (
    !t ||
    !confirm(
      `Delete ${t.package_name}? Existing historical sales will remain, but this ticket will disappear from the POS.`,
    )
  )
    return;
  const { error } = await SELAccess.db()
...
function renderExpenses() {
  ...
  const categories = [
    ...new Set(expenses.map((x) => x.category).filter(Boolean)),
  ].sort();
  const current = cat;
  q("expenseCategoryFilter").innerHTML =
  ...
  const rows = expenses.filter(
    (x) =>
      (current === "all" || x.category === current) &&
      (section === "all" || (x.section || "general") === section) &&
      (!search ||
        norm(
          `${x.expense_reference} ${x.section || "general"} ${x.category} ${x.description} ${x.payment_mode}`,
        ).includes(search)),
  );
  const total = rows.reduce((s, x) => s + Number(x.amount || 0), 0);
  q("expenseTotal").textContent = money(total);
  q("expenseCount").textContent = rows.length.toLocaleString("en-NG");
  q("expenseRows").innerHTML = rows.length
    ? rows
        .map(
          (x) => `
...
function openExpenseEditor(id = null) {
  ...
  q("expenseForm").onsubmit = saveExpense;
  q("cancelExpense").onclick = closeDrawer;
}
...
async function saveExpense(e) {
  ...
  const payload = {
    section: q("expenseSection").value,
    category: q("expenseCategory").value.trim(),
    description: q("expenseDescription").value.trim(),
    amount: Number(q("expenseAmount").value || 0),
    payment_mode: q("expensePayment").value,
    expense_date: new Date(q("expenseDate").value).toISOString(),
    notes: q("expenseNotes").value.trim() || null,
    updated_at: new Date().toISOString(),
  };
  if (!id) payload.recorded_by = adminCtx.user.id;
  const query = id
    ? SELAccess.db().from("expenses").update(payload).eq("id", Number(id))
    : SELAccess.db().from("expenses").insert(payload);
  const { error } = await query;
  btn.disabled = false;
  if (error) return notify(error.message, "error");
  closeDrawer();
  notify(id ? "Expense updated." : "Expense recorded.");
  await loadExpenses();
}
...
async function deleteExpense(id) {
  ...
  if (
    !x ||
    !confirm(`Delete expense ${x.expense_reference} (${money(x.amount)})?`)
  )
    return;
...
async function runFinance() {
  try {
    ...
    notify(err.message || "Could not calculate profit.", "error");
  }
}
...
async function runDetailedFinanceReport(start, end) {
  ...
  if (type === "kitchen" || type === "ticket") {
    const { data, error } = await SELAccess.db()
      .from("sales")
      .select(
        "id,sale_reference,payment_mode,total_amount,sold_by,sale_type,sale_date,created_at,sale_items(item_name,quantity,total_price)",
      )
      .eq("sale_type", type)
    ...
    q("financeDetailTitle").textContent =
      type === "ticket" ? "Ticket Sales Report" : "Kitchen Sales Report";
    q("financeDetailHead").innerHTML =
      "<tr><th>Reference</th><th>Date</th><th>Items</th><th>Payment</th><th>Sold By</th><th>Amount</th></tr>";
    q("financeDetailRows").innerHTML = rows.length
      ? rows
          .map(
            (r) =>
              `<tr><td><strong>${esc(r.sale_reference)}</strong></td><td>${new Date(r.sale_date || r.created_at).toLocaleString("en-NG")}</td><td class="item-summary">${esc((r.sale_items || []).map((i) => `${i.item_name} × ${i.quantity}`).join(", ") || "—")}</td><td>${esc(r.payment_mode)}</td><td>${esc(names[r.sold_by] || "User")}</td><td><strong>${money(r.total_amount)}</strong></td></tr>`,
          )
          .join("")
    ...
    q("financeDetailHead").innerHTML =
      "<tr><th>Receipt</th><th>Date</th><th>Payer / Group</th><th>Visitors</th><th>Payment</th><th>Amount</th></tr>";
    q("financeDetailRows").innerHTML = rows.length
      ? rows
          .map(
            (r) =>
              `<tr><td><strong>${esc(r.receipt_no)}</strong></td><td>${new Date(r.entry_at).toLocaleString("en-NG")}</td><td>${esc(r.payer_name || "—")}</td><td>${Number(r.persons_count || 0).toLocaleString("en-NG")}</td><td>${esc(r.payment_mode)}</td><td><strong>${money(r.amount)}</strong></td></tr>`,
          )
          .join("")
      : '<tr><td colspan="6" class="empty">No records for this period.</td></tr>';
  } else {
    ...
    q("financeDetailHead").innerHTML =
      "<tr><th>Reference</th><th>Date</th><th>Category</th><th>Description</th><th>Payment</th><th>Amount</th></tr>";
    q("financeDetailRows").innerHTML = rows.length
      ? rows
          .map(
            (r) =>
              `<tr><td><strong>${esc(r.expense_reference)}</strong></td><td>${new Date(r.expense_date).toLocaleString("en-NG")}</td><td>${esc(r.category)}</td><td>${esc(r.description)}</td><td>${esc(r.payment_mode)}</td><td><strong>${money(r.amount)}</strong></td></tr>`,
          )
          .join("")
      : '<tr><td colspan="6" class="empty">No records for this period.</td></tr>';
  }
...
function renderInventory() {
  ...
  const rows = inventory.filter(
    (p) =>
      ...
      (!search || norm(`${p.name} ${p.sku} ${p.category}`).includes(search)),
  );
  q("inventoryRows").innerHTML = rows.length
    ? rows
        .map((p) => {
      ...
      <tr>
        ...
        .join("")
    : '<tr><td colspan="8" class="empty">No kitchen products found.</td></tr>';

  q("inventoryRows")
    .querySelectorAll("[data-product-stock]")
    .forEach(
      (btn) => (btn.onclick = () => openStockAdder(btn.dataset.productStock)),
    );
  q("inventoryRows")
    .querySelectorAll("[data-product-reduce]")
    .forEach(
      (btn) => (btn.onclick = () => openStockReducer(btn.dataset.productReduce)),
    );
  q("inventoryRows")
    .querySelectorAll("[data-product-edit]")
    .forEach(
      (btn) => (btn.onclick = () => openProductEditor(btn.dataset.productEdit)),
    );
  q("inventoryRows")
    .querySelectorAll("[data-product-delete]")
    .forEach(
      (btn) => (btn.onclick = () => deleteProduct(btn.dataset.productDelete)),
    );
}
...
function openProductEditor(id = null) {
  ...
  openDrawer(
    p ? "Edit Kitchen Product" : "New Kitchen Product",
    p
      ? "Update product details. Use Add Stock for replenishment."
      : "Create a Kitchen product and set its opening stock.",
    `
...
function openStockAdder(id) {
  ...
  if (!p)
    return notify(
      "Product could not be found. Refresh inventory and try again.",
      "error",
    );
  const current = Number(p.quantity || 0);
  openDrawer(
    "Add Kitchen Stock",
    "Record a new stock delivery without overwriting the existing quantity.",
    `
...
async function addInventoryStock(e) {
  ...
  if (!Number.isInteger(quantity) || quantity <= 0)
    return notify("Enter a whole quantity greater than zero.", "error");
  btn.disabled = true;
  btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Adding Stock…';
  const { data, error } = await SELAccess.db().rpc("add_inventory_stock", {
    p_product_id: id,
    p_quantity: quantity,
    p_note: note,
  });
  ...
  closeDrawer();
  notify(
    `Stock added successfully. New balance: ${Number(result?.new_quantity ?? 0).toLocaleString("en-NG")}.`,
  );
  await loadInventory();
}
...
async function deleteProduct(id) {
  ...
  if (
    !p ||
    !confirm(`Delete ${p.name}? Historical sale records remain available.`)
  )
    return;
  const { error } = await SELAccess.db()
    .from("inventory_items")
...
async function renderAudit() {
  ...
  const rows = audits.filter(
    (a) =>
      !s ||
      norm(
        `${a.product_name || a.ticket_name || ""} ${a.action || ""}`,
      ).includes(s),
  );
  const map = await profileMap(rows.map((x) => x.changed_by));
  q("auditRows").innerHTML = rows.length
    ? rows
        .map((a) => {
        ...
        .join("")
    : '<tr><td colspan="6" class="empty">No audit entries found.</td></tr>';
}
...
async function loadUserData() {
  ...
  users = p.data || [];
  userOverrides = u.data || [];
  rolePermissions = r.data || [];
  renderUsers();
}

function effectivePermissions(user) {
  if (norm(user.role) === "admin" || norm(user.role) === "super_admin")
    return Object.keys(SELAccess.labels);
  ...
  over.forEach((o) =>
    o.granted ? set.add(o.permission_key) : set.delete(o.permission_key),
  );
  return [...set];
}

function renderUsers() {
  q("userRows").innerHTML = users.length
    ? users
        .map((u) => {
        ...
        .join("")
    : '<tr><td colspan="5" class="empty">No user profiles found.</td></tr>';
}
...
function openCreateUser() {
  openDrawer(
    "Create User",
    "Create a Supabase login and assign operational access.",
    `
  ...
  q("newUserRole").onchange = (e) => {
    ...
    q("createPermissionGrid").innerHTML = permissionGrid(
      role === "admin" || role === "super_admin"
        ? Object.keys(SELAccess.labels)
        : roleDefaultPermissions(role),
      role === "admin",
    );
  };
  q("createUserForm").onsubmit = createUser;
  q("cancelUser").onclick = closeDrawer;
}
...
async function createUser(e) {
  ...
  try {
    const { data, error } = await SELAccess.db().functions.invoke(
      "admin-create-user",
      {
        body: {
          full_name: q("newUserName").value.trim(),
          email: q("newUserEmail").value.trim(),
          password: q("newUserPassword").value,
          role: q("newUserRole").value,
          permissions,
        },
      },
    );
    ...
    closeDrawer();
    notify("User created and access assigned.");
    await loadUserData();
  } catch (err) {
    notify("Could not create user: " + err.message, "error");
  } finally {
    btn.disabled = false;
  }
}
...
function bindEvents() {
  ...
  document
    ...
    .forEach((b) => (b.onclick = () => showPanel(b.dataset.jump)));
  ...
  q("dashboardRange").onchange = loadDashboard;
  ...
  q("gatePaymentFilter").onchange = renderGateAdmin;
  q("gateAdminRange").onchange = () => {
    document
      .querySelectorAll(".gate-admin-custom")
      .forEach((x) =>
        x.classList.toggle("hidden", q("gateAdminRange").value !== "custom"),
      );
    if (q("gateAdminRange").value !== "custom") loadGateAdmin();
  };
  q("gateAdminStart").onchange = () => {
    if (
      q("gateAdminRange").value === "custom" &&
      q("gateAdminStart").value &&
      q("gateAdminEnd").value
    )
      loadGateAdmin();
  };
  q("gateAdminEnd").onchange = () => {
    if (
      q("gateAdminRange").value === "custom" &&
      q("gateAdminStart").value &&
      q("gateAdminEnd").value
  ...
  q("newTicket").onclick = () => openTicketEditor();
  q("reloadTickets").onclick = loadTickets;
  q("ticketSearch").oninput = renderTickets;
  q("ticketAudienceFilter").onchange = renderTickets;
  q("newExpense").onclick = () => openExpenseEditor();
  q("reloadExpenses").onclick = loadExpenses;
  q("expenseSearch").oninput = renderExpenses;
  q("expenseCategoryFilter").onchange = renderExpenses;
  q("expenseSectionFilter").onchange = renderExpenses;
  q("financeRange").onchange = () =>
    document
      .querySelectorAll(".finance-custom")
      .forEach((x) =>
        x.classList.toggle("hidden", q("financeRange").value !== "custom"),
      );
  q("financeReportType").onchange = runFinance;
  q("runFinance").onclick = runFinance;
  q("newProduct").onclick = () => openProductEditor();
